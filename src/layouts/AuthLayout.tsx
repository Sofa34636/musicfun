...
export const AuthLayout = () => {
    return <div>
        ...
        <Outlet /> {/* сюда будет вставляться логин либо регистрация */}
...
const NavbarMenuItem = ({to, title}) => {
    return (
        <NavLink className={({ isActive }) =>
            isActive ? styles.active : ""} to={to}>{title}</NavLink>  // NavLink - задача поменять адресную строку браузера, не делая настоящую перегрузку страницы
)
}
...
export const GlobalLayout = () => {
    ...
    let lang = params['lang'] // lang - язык
    if (!lang) lang = 'ge'
